Form2Json = {
      ...
      if(keys_match) {
        ...
        $.each(keys_match_ary, function (k, v) {
          ...
          if(key == '') {
            ...
            key = Form2Json.getPushKey(pointer);
          }
  ...
  getFormDataJson: function(form, jsonVar) {
    if(jsonVar == undefined) {
      jsonVar = '__JSON';
    }

    var formData = Form2Json.getFormData(form);
    var packedFormData = {}
  ...
  submitFormAsJson: function(originalForm, jsonVar) {
    if(jsonVar == undefined) {
      jsonVar = '__JSON';
    }
    ...
    $hidden.val(JSON.stringify(Form2Json.getFormData(originalForm)));
